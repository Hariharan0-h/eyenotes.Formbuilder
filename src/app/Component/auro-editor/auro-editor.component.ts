...
// Then in your component class:
  ...
  startDrag(e: MouseEvent, element: HTMLElement): void {
    ...
    // Get current position from style if set, otherwise from getBoundingClientRect
    let left = 0;
    let top = 0;
    
    // Check if element has position data in style
  ...
  handleMouseMove = (e: MouseEvent): void => {
    if (this.isDragging && this.selectedObject && this.selectedObject.element) {
      ...
      let leftOffset = parseInt(element.style.left || '0');
      let topOffset = parseInt(element.style.top || '0');
      ...
      if (this.selectedObject.type === 'image') {
        ...
        switch(this.resizeHandle) {
          ...
          case 'sw':
            newWidth = Math.max(50, this.resizeStartWidth - deltaX);
            newHeight = Math.max(50, this.resizeStartHeight + deltaY);
            leftOffset = parseInt(element.style.left || '0') + deltaX;
            break;
          case 'ne':
            newWidth = Math.max(50, this.resizeStartWidth + deltaX);
            newHeight = Math.max(50, this.resizeStartHeight - deltaY);
            topOffset = parseInt(element.style.top || '0') + deltaY;
            break;
          case 'nw':
            newWidth = Math.max(50, this.resizeStartWidth - deltaX);
            newHeight = Math.max(50, this.resizeStartHeight - deltaY);
...
<head>
  ...
  <style>
    ...
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+Devanagari:wght@400;500;700&family=Poppins:wght@400;500;700&display=swap');
...
<body>
  <div class="content ${this.currentLanguage}">
    ${this.getAllPagesContent()}
  </div>
</body>
</html>`;
  }
